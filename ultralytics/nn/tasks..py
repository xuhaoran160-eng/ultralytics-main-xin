...
class SACRSpeciesSegmentationLoss(v8SegmentationLoss):
    """
    Tree-preserving segmentation loss with delayed, detached MS-MLP species supervision.

    Main principles:
        1) Main YOLO loss is tree-only: original tp1/tp2 labels are folded to tree=0.
        2) SACR refined mask is supervised by the official segmentation loss.
        3) Species classification starts only after head.species_start_epoch, default 20.
        4) Species losses use detached features in head.py, so they update only the
           species MLP/projection parameters and do not backpropagate into YOLO
           backbone/neck/box/cls/mask branches.
    """
    ...
    def __init__(self, model):
        ...
        self.model = model
        head = model.model[-1]
        self.species_nc = int(getattr(head, "species_nc", 2))
        self.species_gain = float(getattr(head, "species_gain", 0.10))
        self.object_species_gain = float(getattr(head, "object_species_gain", 0.20))

    def _head(self):
        return self.model.model[-1]

    def _species_is_active(self) -> bool:
        head = self._head()
        cur_epoch = int(getattr(head, "_current_epoch", 0))
        start_epoch = int(getattr(head, "species_start_epoch", 20))
        return cur_epoch >= start_epoch

    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        # Keep original tp1/tp2 labels for auxiliary species losses.
        original_cls = batch["cls"].clone()

        # Main YOLO branch remains tree-only: collapse all GT classes to tree class 0.
        batch_tree = dict(batch)
        batch_tree["cls"] = torch.zeros_like(batch["cls"])

        # Official segmentation loss supervises box + tree cls + refined mask + dfl.
        loss, loss_items = super().loss(preds, batch_tree)

        # Do not train species classifier before the scheduled epoch.
        if not self._species_is_active():
            return loss, loss_items

        head = self._head()
        sp_loss = torch.zeros((), device=self.device)

        if bool(getattr(head, "anchor_species_loss", True)) and float(getattr(head, "species_gain", self.species_gain)) > 0:
            sp_loss = sp_loss + self._species_aux_loss(preds, batch, batch_tree, original_cls)

        if bool(getattr(head, "object_species_loss", True)) and float(getattr(head, "object_species_gain", self.object_species_gain)) > 0:
            sp_loss = sp_loss + self._object_species_loss(preds, batch, original_cls)

        if torch.is_tensor(loss) and loss.ndim > 0 and loss.numel() > 1:
            batch_size = preds["boxes"].shape[0]
            # Keep compatibility with Ultralytics segment loss layout by adding aux CE into cls slot.
            # Because species features are detached in head.py, this does not update YOLO main branches.
            loss[1] = loss[1] + sp_loss * batch_size
            if torch.is_tensor(loss_items) and loss_items.numel() > 1:
                loss_items = loss_items.clone()
                loss_items[1] = loss_items[1] + sp_loss.detach()
        else:
            loss = loss + sp_loss * preds["boxes"].shape[0]
        return loss, loss_items

    def _species_aux_loss(self, preds, batch, batch_tree, original_cls):
        """Anchor-level CE for MLP species_logits. Gradients are blocked from YOLO by detach in head.py."""
        head = self._head()
        species_logits = preds.get("species_logits", None)
        if species_logits is None:
            return torch.zeros((), device=self.device)

        species_gain = float(getattr(head, "species_gain", self.species_gain))
        if species_gain <= 0:
            return species_logits.sum() * 0.0

        # species_logits: [B, species_nc, A] -> [B, A, species_nc]
        species_logits = species_logits.permute(0, 2, 1).contiguous()
        batch_size = species_logits.shape[0]

        if original_cls.numel() == 0:
            return species_logits.sum() * 0.0

        # Reuse YOLO assignment with tree-only labels to know which anchors correspond to which GT.
        with torch.no_grad():
            (fg_mask, target_gt_idx, _, _, _), _, _ = self.get_assigned_targets_and_loss(preds, batch_tree)
        ...
        targets = torch.cat((batch["batch_idx"].view(-1, 1), original_cls.view(-1, 1), batch["bboxes"]), 1)
        targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        if targets.shape[1] == 0:
            return species_logits.sum() * 0.0

        gt_species = targets[..., 0].long().clamp_(0, self.species_nc - 1)  # [B, max_gt]
        safe_idx = target_gt_idx.long().clamp_(0, gt_species.shape[1] - 1)
        assigned_species = gt_species.gather(1, safe_idx)  # [B, A]

        pred_fg = species_logits[fg_mask]
        target_fg = assigned_species[fg_mask]
        if pred_fg.numel() == 0:
            return species_logits.sum() * 0.0

        return species_gain * F.cross_entropy(pred_fg.float(), target_fg.long(), reduction="mean")

    def _object_species_loss(self, preds, batch, original_cls):
        """Object-level CE using GT masks and P3/P4/P5 mask-guided pooled features."""
        head = self._head()
        if not hasattr(head, "object_species_logits_from_masks"):
            return torch.zeros((), device=self.device)

        if "feats" not in preds or "masks" not in batch:
            return torch.zeros((), device=self.device)
        ...
        labels = original_cls.view(-1).long().to(self.device)
        if labels.numel() == 0:
            return torch.zeros((), device=self.device)

        # Expected when overlap_mask=False: masks: [N, H, W].
        if masks.ndim != 3 or masks.shape[0] != labels.shape[0]:
            return preds["feats"][0].sum() * 0.0

        batch_idx = batch["batch_idx"].view(-1).long().to(self.device)
        if batch_idx.numel() != labels.numel():
            return preds["feats"][0].sum() * 0.0

        logits = head.object_species_logits_from_masks(
            preds["feats"],
            masks.to(self.device).float(),
            batch_idx=batch_idx,
        )

        n = min(int(logits.shape[0]), int(labels.shape[0]))
        if n <= 0:
            return preds["feats"][0].sum() * 0.0

        logits = logits[:n]
        labels = labels[:n].clamp_(0, self.species_nc - 1)
        gain = float(getattr(head, "object_species_gain", self.object_species_gain))
        return gain * F.cross_entropy(logits.float(), labels, reduction="mean")
...
class SegmentationModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        head = self.model[-1]
        if head.__class__.__name__ == "SACRSpeciesSegment":
            if getattr(self, "end2end", False):
                return E2ELoss(self, SACRSpeciesSegmentationLoss)
            return SACRSpeciesSegmentationLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is SACRSpeciesSegment:
                # SACRSpeciesSegment args = [tree_nc, species_nc, nm, npr, reg_max, end2end, ch]
                args[3] = make_divisible(min(args[3], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, SACRSpeciesSegment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
        elif m is v10Detect:
...
def guess_model_task(model):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, SACRSpeciesSegment, YOLOESegment)):
                return "segment"
