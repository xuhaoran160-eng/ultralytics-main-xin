...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "SACRSpeciesSegment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class SACRSpeciesSegment(Segment):
    """
    SACR-fenlei2 with MS-MLP species classifier.

    Design used here:
        1) Main YOLO segmentation branch is kept tree-preserving.
           The main cv3 branch is NOT replaced. In loss, GT cls is folded to tree=0.
        2) SACR mask refinement remains local and unchanged in principle:
              refined_coeff = base_coeff + effective_refine_alpha * gate * delta
           effective_refine_alpha is scheduled by epoch:
              epoch < 20: 0; epoch 20-40: linear 0 -> refine_alpha; epoch >= 40: refine_alpha.
           The local refinement branch is supervised only by normal mask loss.
        3) Final species logits are produced by a detached multi-scale MLP classifier.
           It uses P3/P4/P5 projected features, aligned to each scale, then an MLP
           similar to ResNet classifier tail: pooled/vector feature -> FC -> species logits.
        4) Object-level species loss also uses P3/P4/P5 + GT masks with mask-guided
           average pooling, then the same MLP classifier.
        5) To block species head from damaging segmentation, all species-classification
           features are detached by default. Gradients from species losses update only
           ms_species_proj and obj_species_mlp, not backbone/neck/mask/box/cls branches.

    YAML args:
        [tree_nc, species_nc, nm, npr]
        Example: [1, 2, 32, 256]
    """
    ...
    def __init__(
        ...
        end2end=False,
        ch: tuple = (),
        refine_alpha: float = 0.03,
        species_gain: float = 0.10,
        object_species_gain: float = 0.20,
        object_feature_detach: bool = True,
        species_cls_detach: bool = True,
        species_start_epoch: int = 20,
        refine_start_epoch: int = 20,
        refine_warmup_epochs: int = 20,
        anchor_species_loss: bool = True,
        object_species_loss: bool = True,
        species_mlp_hidden: int = 256,
        species_mlp_dropout: float = 0.10,
        local_gap_kernel: int = 3,
    ):
        # Keep public YOLO output compatible with dataset nc (= species_nc, usually 2).
        # Main segmentation loss will collapse all GT classes to class 0 = tree.
        super().__init__(species_nc, nm, npr, reg_max, end2end, ch)
        self.tree_nc = int(tree_nc)
        self.species_nc = int(species_nc)
        self.refine_alpha = float(refine_alpha)
        self.species_gain = float(species_gain)
        self.object_species_gain = float(object_species_gain)
        self.object_feature_detach = bool(object_feature_detach)
        self.species_cls_detach = bool(species_cls_detach)
        self.species_start_epoch = int(species_start_epoch)
        self.refine_start_epoch = int(refine_start_epoch)
        self.refine_warmup_epochs = int(refine_warmup_epochs)
        self.anchor_species_loss = bool(anchor_species_loss)
        self.object_species_loss = bool(object_species_loss)
        self.local_gap_kernel = int(local_gap_kernel)
        # Default to full refinement for normal inference if no trainer callback sets the epoch.
        # The training script sets this back to 0 before training starts.
        self._current_epoch = int(self.refine_start_epoch + max(self.refine_warmup_epochs, 0))

        # ------------------------------------------------------------
        # A. Local SACR refinement branch.
        # This branch keeps the original local SACR idea and is used for mask coefficient correction.
        # It is intentionally separated from the final species MLP branch.
        # ------------------------------------------------------------
        csp = max(ch[0], min(self.species_nc, 100))
        self.cv_sacr_feat = (
            nn.ModuleList(nn.Sequential(Conv(x, csp, 3), Conv(csp, csp, 3)) for x in ch)
            if self.legacy
            else nn.ModuleList(
                nn.Sequential(
                    nn.Sequential(DWConv(x, x, 3), Conv(x, csp, 1)),
                    nn.Sequential(DWConv(csp, csp, 3), Conv(csp, csp, 1)),
                )
                for x in ch
            )
        )
        # Backward-compatible alias. Do not use this for final species prediction.
        self.cv_species_feat = self.cv_sacr_feat

        self.sacr_delta = nn.ModuleList(nn.Conv2d(csp, self.nm, 1) for _ in ch)
        self.sacr_gate = nn.ModuleList(nn.Sequential(nn.Conv2d(csp, self.nm, 1), nn.Sigmoid()) for _ in ch)

        # Stability-first initialization: epoch 0 is almost original YOLO-seg.
        for m in self.sacr_delta:
            nn.init.zeros_(m.weight)
            nn.init.zeros_(m.bias)
        for m in self.sacr_gate:
            nn.init.constant_(m[0].bias, -2.0)  # sigmoid(-2) ≈ 0.12, weak initial refinement

        # ------------------------------------------------------------
        # B. Detached multi-scale MLP species classifier.
        # It uses P3/P4/P5, but does not send gradients back to YOLO features by default.
        # Each scale is projected to cobj, resized to a target scale, concatenated,
        # then classified by a Linear MLP. The same MLP is used for:
        #   - anchor/location species logits for standard YOLO inference;
        #   - object-level logits after GT-mask-guided average pooling during training.
        # ------------------------------------------------------------
        cobj = int(max(64, min(ch[0], int(species_mlp_hidden))))
        self.cobj = cobj
        self.ms_species_proj = nn.ModuleList(Conv(x, cobj, 1) for x in ch)
        self.obj_pool_proj = self.ms_species_proj  # backward-compatible name
        self.obj_species_mlp = nn.Sequential(
            nn.Linear(cobj * self.nl, cobj),
            nn.SiLU(),
            nn.Dropout(p=float(species_mlp_dropout)),
            nn.Linear(cobj, self.species_nc),
        )

        if end2end:
            self.one2one_cv_sacr_feat = copy.deepcopy(self.cv_sacr_feat)
            self.one2one_sacr_delta = copy.deepcopy(self.sacr_delta)
            self.one2one_sacr_gate = copy.deepcopy(self.sacr_gate)
            self.one2one_ms_species_proj = copy.deepcopy(self.ms_species_proj)
            self.one2one_obj_species_mlp = copy.deepcopy(self.obj_species_mlp)

    @property
    def one2many(self):
        """One-to-many training/inference heads."""
        return dict(
            box_head=self.cv2,
            cls_head=self.cv3,                 # main tree branch; supervised as tree=0 in tasks.py
            mask_head=self.cv4,                # base mask coefficients
            sacr_feat_head=self.cv_sacr_feat,  # local SACR feature for coefficient refinement
            delta_head=self.sacr_delta,
            gate_head=self.sacr_gate,
            species_proj_head=self.ms_species_proj,
            species_mlp_head=self.obj_species_mlp,
        )

    @property
    def one2one(self):
        """One-to-one heads for end2end variants."""
        return dict(
            box_head=self.one2one_cv2,
            cls_head=self.one2one_cv3,
            mask_head=self.one2one_cv4,
            sacr_feat_head=self.one2one_cv_sacr_feat,
            delta_head=self.one2one_sacr_delta,
            gate_head=self.one2one_sacr_gate,
            species_proj_head=self.one2one_ms_species_proj,
            species_mlp_head=self.one2one_obj_species_mlp,
        )

    def _apply_species_mlp_to_map(self, feat_map: torch.Tensor, mlp: torch.nn.Module | None = None) -> torch.Tensor:
        """Apply Linear MLP to each spatial location of a [B,C,H,W] feature map."""
        if mlp is None:
            mlp = self.obj_species_mlp
        b, c, h, w = feat_map.shape
        v = feat_map.permute(0, 2, 3, 1).contiguous().view(b * h * w, c)
        logits = mlp(v)
        return logits.view(b, h, w, self.species_nc).permute(0, 3, 1, 2).contiguous()

    def _project_multiscale_for_species(self, x: list[torch.Tensor], species_proj_head=None, detach: bool | None = None):
        """Project P3/P4/P5 to cobj channels for species classification."""
        if species_proj_head is None:
            species_proj_head = self.ms_species_proj
        if detach is None:
            detach = self.species_cls_detach
        feats = [xi.detach() if detach else xi for xi in x]
        return [species_proj_head[i](feats[i]) for i in range(self.nl)]

    def _anchor_species_logits_from_multiscale(
        self,
        x: list[torch.Tensor],
        species_proj_head=None,
        species_mlp_head=None,
        detach: bool | None = None,
    ) -> torch.Tensor:
        """
        Produce anchor/location species logits using detached P3/P4/P5 multi-scale features.

        For each target scale i:
            1) project P3/P4/P5 -> cobj;
            2) resize all projected features to scale i resolution;
            3) concatenate them to [B, 3*cobj, Hi, Wi];
            4) optional local average pooling;
            5) Linear MLP per location -> [B, species_nc, Hi, Wi].
        """
        if species_mlp_head is None:
            species_mlp_head = self.obj_species_mlp
        proj = self._project_multiscale_for_species(x, species_proj_head, detach=detach)
        outs = []
        for i in range(self.nl):
            ...
            for p in proj:
                if p.shape[2:] != size:
                    aligned.append(F.interpolate(p, size=size, mode="bilinear", align_corners=False))
                else:
                    aligned.append(p)
            cat = torch.cat(aligned, dim=1)
            k = int(getattr(self, "local_gap_kernel", 3))
            if k > 1:
                if k % 2 == 0:
                    k += 1
                cat = F.avg_pool2d(cat, kernel_size=k, stride=1, padding=k // 2)
            logit_map = self._apply_species_mlp_to_map(cat, species_mlp_head)
            outs.append(logit_map.view(logit_map.shape[0], self.species_nc, -1))
        return torch.cat(outs, dim=2)

    def _get_effective_refine_alpha(self) -> float:
        """Scheduled SACR refinement strength.

        Epoch schedule:
            epoch < refine_start_epoch: 0
            refine_start_epoch <= epoch < refine_start_epoch + refine_warmup_epochs:
                linear warm-up from 0 to self.refine_alpha
            epoch >= refine_start_epoch + refine_warmup_epochs:
                self.refine_alpha
        """
        target_alpha = float(getattr(self, "refine_alpha", 0.03))
        start = int(getattr(self, "refine_start_epoch", 20))
        warm = int(getattr(self, "refine_warmup_epochs", 20))
        cur = int(getattr(self, "_current_epoch", start + max(warm, 0)))
        ...
        if warm <= 0:
            return target_alpha
        if cur < start + warm:
            ratio = float(cur - start) / float(warm)
            ratio = max(0.0, min(1.0, ratio))
            return target_alpha * ratio
        return target_alpha
    ...
    def forward_head(
        ...
        sacr_feat_head: torch.nn.Module,
        delta_head: torch.nn.Module,
        gate_head: torch.nn.Module,
        species_proj_head: torch.nn.Module = None,
        species_mlp_head: torch.nn.Module = None,
    ) -> dict[str, torch.Tensor]:
        """Return tree boxes/scores, MLP species logits, and refined mask coefficients."""
        ...
        # Local SACR coefficient refinement. This part is deliberately unchanged in principle,
        # but the refinement strength is warmed up by epoch to prevent early mask merging.
        effective_refine_alpha = self._get_effective_refine_alpha()
        for i in range(self.nl):
            base_coeff = mask_head[i](x[i])
            sacr_feat = sacr_feat_head[i](x[i])
            delta = delta_head[i](sacr_feat)
            gate = gate_head[i](sacr_feat)
            refined_coeff = base_coeff + effective_refine_alpha * gate * delta
            refined_coeffs.append(refined_coeff.view(bs, self.nm, -1))

        preds["effective_refine_alpha"] = torch.as_tensor(effective_refine_alpha, device=x[0].device, dtype=x[0].dtype)

        preds["mask_coefficient"] = torch.cat(refined_coeffs, 2)

        # Final species prediction logits now come from detached multi-scale MLP result.
        # This prevents species classification loss from updating the YOLO main segmentation branch.
        preds["species_logits"] = self._anchor_species_logits_from_multiscale(
            x,
            species_proj_head=species_proj_head,
            species_mlp_head=species_mlp_head,
            detach=self.species_cls_detach,
        )
        return preds

    def object_species_logits_from_masks(
        self,
        feats: list[torch.Tensor] | torch.Tensor,
        masks: torch.Tensor,
        batch_idx: torch.Tensor | None = None,
        detach_feature: bool | None = None,
        eps: float = 1e-6,
    ) -> torch.Tensor:
        """
        Object-level species logits by GT-mask-guided multi-scale pooling.

        Inputs:
            feats: P3/P4/P5 feature maps, usually preds["feats"], list of [B,C,H,W].
            masks: GT instance masks, [N,H,W].
        Output:
            logits: [N, species_nc].

        This branch is training-only in the standard Ultralytics pipeline. Since features are
        detached by default, gradients update only ms_species_proj and obj_species_mlp.
        """
        if masks is None:
        ...
        if not isinstance(feats, (list, tuple)) or len(feats) == 0:
            return torch.zeros((0, self.species_nc), device=masks.device, dtype=masks.dtype)

        if masks.ndim == 2:
            masks = masks[None]
        masks = masks.float().to(feats[0].device)
        n = int(masks.shape[0])
        if n == 0:
            return torch.zeros((0, self.species_nc), device=feats[0].device, dtype=feats[0].dtype)

        if batch_idx is None:
            batch_idx = torch.zeros(n, device=feats[0].device, dtype=torch.long)
        else:
            batch_idx = batch_idx.to(feats[0].device).long().view(-1)
            if batch_idx.numel() != n:
                batch_idx = torch.zeros(n, device=feats[0].device, dtype=torch.long)

        if detach_feature is None:
            detach_feature = self.object_feature_detach

        proj = self._project_multiscale_for_species(list(feats), self.ms_species_proj, detach=detach_feature)
        pooled_scales = []
        for p in proj:
            _, c, hf, wf = p.shape
            masks_ds = F.interpolate(masks[:, None], size=(hf, wf), mode="bilinear", align_corners=False).clamp_(0.0, 1.0)
            pooled = []
            for j in range(n):
                b = int(batch_idx[j].item())
                if b < 0 or b >= p.shape[0]:
                    b = 0
                m = masks_ds[j]  # [1,Hf,Wf]
                denom = m.sum().clamp_min(eps)
                v = (p[b] * m).sum(dim=(1, 2)) / denom
                pooled.append(v)
            pooled_scales.append(torch.stack(pooled, dim=0) if pooled else torch.zeros((0, c), device=p.device, dtype=p.dtype))

        obj_vec = torch.cat(pooled_scales, dim=1)  # [N, nl*cobj]
        mlp_dtype = next(self.obj_species_mlp.parameters()).dtype
        logits = self.obj_species_mlp(obj_vec.to(dtype=mlp_dtype))
        return logits.to(feats[0].dtype)
    ...
    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Output format:
            [boxes(4), species-class-scores(species_nc), refined-mask-coefficients(nm)]

        Species class is produced by the detached multi-scale MLP logits in x["species_logits"].
        Confidence still comes from the main YOLO tree score.
        """
        dbox = Detect._get_decode_boxes(self, x)
        main_scores = x["scores"].sigmoid()
        tree_score = main_scores[:, :1, :] if main_scores.shape[1] >= 1 else main_scores.max(dim=1, keepdim=True).values

        species_logits = x.get("species_logits", None)
        if species_logits is None:
            scores = main_scores
        else:
            species_prob = species_logits.float().softmax(dim=1)
            species_cls = species_prob.argmax(dim=1, keepdim=True)
            scores = torch.zeros_like(species_prob)
            scores.scatter_(1, species_cls, tree_score.to(scores.dtype))
        return torch.cat((dbox, scores, x["mask_coefficient"]), dim=1)

    def fuse(self) -> None:
        """Remove one-to-many heads for inference optimization."""
        super().fuse()
        self.cv_sacr_feat = self.cv_species_feat = self.sacr_delta = self.sacr_gate = None
        self.ms_species_proj = self.obj_pool_proj = self.obj_species_mlp = None
